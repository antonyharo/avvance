...
export default function JobsSkeleton({ loading, data = mockJobsData }) {
  if (loading) {
    // número de skeletons visíveis
    ...
    return (
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {placeholderItems.map((_, i) => (
          <div
            key={i}
            className="p-4 rounded-xl border bg-card shadow-sm animate-pulse flex flex-col gap-3"
          >
